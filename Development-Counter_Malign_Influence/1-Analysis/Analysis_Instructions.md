
#### NOTE
Use the Developer Workbook when possible from the Download [Link](https://u.pcloud.link/publink/show?code=XZXi12VZlK3RSPcAJ7ytQDx7dzxnEjcav29V) 

## Problem Statement
(TP 350-70-14, Par  3-7)
"Government employees are tasked with operating in the information environment but lack the skills and knowledge to identify and counter misinformation and disinformation campaigns effectively. This includes challenges in recognizing propaganda, deepfakes, and malign influence. The performance gap is evident in the employees' inability to:
Accurately discern between reliable information and misinformation.
Detect and respond to disinformation campaigns.
Identify and mitigate the impact of propaganda and malign influence.
Recognize and counter deepfakes and other advanced manipulative media tactics.
This deficiency hampers their ability to perform their duties effectively in the information environment, leading to potential risks and vulnerabilities in their operations.


"
## Scope of the Performance Problem:
(TP 350-70-14, Par 3-9)
"This issue is widespread among government employees who are required to operate in the information environment. The gravity of the problem is significant, with serious implications for national security, public trust, and operational effectiveness. The inability to effectively identify and counter misinformation and disinformation can lead to:
Increased vulnerability to foreign and domestic malign influence operations.
Compromised decision-making processes based on inaccurate or manipulated information.
Erosion of public trust in government communications and operations.
Potential for widespread dissemination of harmful misinformation and propaganda, undermining democratic processes and national security.
This performance gap affects the overall mission readiness and effectiveness of government employees tasked with maintaining the integrity of information in the public sphere.


"
## Needs Analysis (DOTmLPF-P)
(TP 350-70-14, Par 3-3, Table 3-1)
Perform a DOTMLPF Analysis on your problem statement:  
   • Determine whether or not each DOTMLPF domain applies to your problem statement.  For each applicable domain:  
            − Identify the current state of the domain as it pertains to your problem statement and  
            − Propose actions that may alleviate the problem  
   • Answer the three questions on the DOTMLPF tab that are below the DOTMLPF analysis to prepare for goal analysis.

## Goal/Topic Analysis-(as required)

### Major Topics:

Identifying Misinformation and Disinformation
Countering Misinformation and Disinformation
Identifying Rival Influence
Mitigating Rival Influence
Recognizing AI Generated Content
Reporting Malign Influence

### Minor Topics:

Techniques for discerning reliable information
Strategies for detecting disinformation campaigns
Methods to identify propaganda and malign influence
Tools for recognizing and countering deepfakes
Best practices for creating AI-generated content
Protocols for reporting malign influence

## Task Analysis / Total Tasks Inventory (TTI)

0001	Identifying Misinformation and Disinformation
0002	Countering Misinformation and Disinformation
0003	Identifying Rival Influence
0004	Mitigating Rival Influence
0005	Creating AI Generated Content
0006	Recognize AI Generated Content
0007	Reporting Malign Influence
## Difficulty-Importance-Frequency (DIF):

Perform a Difficulty-Importance-Frequency (DIF) Model analysis on the tasks listed in the TTI to identify potential critical tasks. Update total task inventory (TTI) with potential critical tasks to present to CTSSB (5 tasks).

### Individual Task Analysis

#### Task 0001: Identify Misinformation and Disinformation

- **Task Condition Statement**: Given access to various information sources, including digital and print media, government employees need to identify and discern between reliable information and misinformation or disinformation.
- **Task Standard Statement**: Employees must accurately identify misinformation and disinformation, ensuring that at least 95% of assessments are correct and report findings to the relevant authorities.

**Performance Steps**:

1. **Assess the Source of Information**
   - **References**: Guidelines on Source Verification
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: Ensure information is accessed in a secure environment.
   - **Knowledge**: Understanding of credible sources, knowledge of source verification techniques.
   - **Skills**: Analytical skills, attention to detail.
   - **Resources, Materiels**: Access to databases and verification tools.
   - **Step Notes / Cues**: Verify authorship and publication credibility.

2. **Analyze the Content for Bias and Inconsistencies**
   - **References**: Bias Detection Frameworks
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Awareness of common bias indicators, understanding of logical fallacies.
   - **Skills**: Critical thinking, analytical skills.
   - **Resources, Materiels**: Access to content analysis tools.
   - **Step Notes / Cues**: Check for emotive language and unsupported claims.

3. **Cross-Verify Information with Trusted Sources**
   - **References**: Cross-Verification Techniques
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Familiarity with trusted and authoritative sources.
   - **Skills**: Research skills, attention to detail.
   - **Resources, Materiels**: Access to trusted databases and information repositories.
   - **Step Notes / Cues**: Look for corroborative evidence from multiple reliable sources.


#### Task 0003: Identify Rival Influence

- **Task Condition Statement**: Given access to various intelligence reports and media sources, government employees must identify the influence activities of rival entities.
- **Task Standard Statement**: Employees must correctly identify and report on rival influence activities with 90% accuracy.

**Performance Steps**:

1. **Collect and Review Intelligence Reports**
   - **References**: Intelligence Collection Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: Ensure secure handling of classified information.
   - **Knowledge**: Understanding of intelligence sources and methods.
   - **Skills**: Analytical skills, attention to detail.
   - **Resources, Materiels**: Access to intelligence databases and reports.
   - **Step Notes / Cues**: Verify the credibility of intelligence sources.

2. **Analyze Patterns of Influence Activities**
   - **References**: Influence Analysis Frameworks
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Knowledge of influence techniques, understanding of geopolitical context.
   - **Skills**: Critical thinking, pattern recognition.
   - **Resources, Materiels**: Analytical tools and software.
   - **Step Notes / Cues**: Identify common tactics and strategies used by rival entities.

3. **Prepare and Submit Reports**
   - **References**: Reporting Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: Ensure secure transmission of reports.
   - **Knowledge**: Knowledge of reporting formats and requirements.
   - **Skills**: Writing skills, attention to detail.
   - **Resources, Materiels**: Access to reporting tools and platforms.
   - **Step Notes / Cues**: Ensure clarity and accuracy in reporting.


#### Task 0005: Create AI Generated Content

- **Task Condition Statement**: Using AI tools and software, government employees must create AI-generated content for various applications.
- **Task Standard Statement**: Employees must create AI-generated content that meets specified quality standards and is free of errors.

**Performance Steps**:

1. **Select Appropriate AI Tools**
   - **References**: AI Tools Selection Guide
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Understanding of available AI tools and their applications.
   - **Skills**: Technical skills, decision-making skills.
   - **Resources, Materiels**: Access to AI tools and software.
   - **Step Notes / Cues**: Ensure selected tools meet the project requirements.

2. **Input Data and Configure Settings**
   - **References**: Data Input Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: Ensure data privacy and security.
   - **Knowledge**: Knowledge of data input techniques and settings configuration.
   - **Skills**: Technical skills, attention to detail.
   - **Resources, Materiels**: Access to data sources and configuration tools.
   - **Step Notes / Cues**: Verify data accuracy before input.

3. **Generate and Review Content**
   - **References**: Content Review Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Understanding of quality standards for generated content.
   - **Skills**: Review skills, attention to detail.
   - **Resources, Materiels**: Access to review tools and software.
   - **Step Notes / Cues**: Check for errors and quality issues.

#### Task 0006: Recognize AI Generated Content

- **Task Condition Statement**: Given various digital media, government employees must recognize AI-generated content.
- **Task Standard Statement**: Employees must accurately recognize AI-generated content with a 90% accuracy rate.

**Performance Steps**:

1. **Analyze Media Content**
   - **References**: Media Analysis Techniques
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Understanding of AI-generated content characteristics.
   - **Skills**: Analytical skills, attention to detail.
   - **Resources, Materiels**: Access to media analysis tools.
   - **Step Notes / Cues**: Look for common indicators of AI generation.

2. **Use AI Detection Tools**
   - **References**: AI Detection Tools Guide
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Knowledge of available AI detection tools.
   - **Skills**: Technical skills, tool proficiency.
   - **Resources, Materiels**: Access to AI detection software.
   - **Step Notes / Cues**: Ensure tools are updated and configured correctly.

3. **Cross-Verify with Known Authentic Content**
   - **References**: Verification Techniques
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Understanding of content verification methods.
   - **Skills**: Verification skills, attention to detail.
   - **Resources, Materiels**: Access to verified content databases.
   - **Step Notes / Cues**: Compare suspect content with verified examples.



### Exchange (Partner) Sections

#### Objective: Countering Misinformation and Disinformation

- **Task Condition Statement**: In collaboration with a partner organization, participants must develop strategies to counter misinformation and disinformation based on previously learned concepts.
- **Task Standard Statement**: The partner organization must successfully implement strategies that reduce the spread of misinformation and disinformation by 80% within their operational context.

**Performance Steps**:

1. **Collaborate with Partner Organization to Develop a Counter-Strategy Plan**
   - **References**: Strategic Communication Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: Ensure secure communication channels.
   - **Knowledge**: Understanding of strategic communication, knowledge of misinformation tactics.
   - **Skills**: Strategic planning, analytical skills.
   - **Resources, Materials**: Access to strategic planning tools and resources.
   - **Step Notes / Cues**: Align strategy with organizational goals and resources.

2. **Guide Partner Organization in Implementing Counter-Disinformation Measures**
   - **References**: Counter-Disinformation Techniques
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Techniques for countering disinformation, understanding of information dissemination channels.
   - **Skills**: Communication skills, execution skills.
   - **Resources, Materials**: Access to communication platforms and monitoring tools.
   - **Step Notes / Cues**: Monitor the effectiveness of measures and adjust as needed.

3. **Evaluate and Report Effectiveness of Partner's Implementation**
   - **References**: Evaluation Metrics and Reporting Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Understanding of evaluation metrics, knowledge of reporting procedures.
   - **Skills**: Analytical skills, reporting skills.
   - **Resources, Materials**: Access to evaluation tools and reporting systems.
   - **Step Notes / Cues**: Document successes and areas for improvement.

#### Objective: Mitigating Rival Influence

- **Task Condition Statement**: Participants must work with the partner organization to identify and mitigate rival influence activities using the learned techniques.
- **Task Standard Statement**: The partner organization must effectively mitigate identified rival influences, reducing their impact by at least 70%.

**Performance Steps**:

1. **Collaborate with Partner Organization to Develop a Mitigation Plan**
   - **References**: Mitigation Planning Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Understanding of mitigation strategies and techniques.
   - **Skills**: Strategic planning, analytical skills.
   - **Resources, Materials**: Access to planning tools and resources.
   - **Step Notes / Cues**: Align the plan with organizational goals.

2. **Guide Partner Organization in Implementing Mitigation Measures**
   - **References**: Mitigation Techniques
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: Ensure safe execution of measures.
   - **Knowledge**: Knowledge of influence mitigation techniques.
   - **Skills**: Execution skills, communication skills.
   - **Resources, Materials**: Tools and resources for implementing measures.
   - **Step Notes / Cues**: Monitor the implementation process.

3. **Evaluate and Report Effectiveness of Partner's Mitigation Efforts**
   - **References**: Evaluation Metrics and Reporting Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Understanding of evaluation metrics.
   - **Skills**: Analytical skills, reporting skills.
   - **Resources, Materials**: Access to evaluation tools and reporting systems.
   - **Step Notes / Cues**: Document the impact and effectiveness of mitigation measures.

#### Objective: Reporting Malign Influence

- **Task Condition Statement**: Participants must work with the partner organization to establish and implement protocols for reporting instances of malign influence.
- **Task Standard Statement**: The partner organization must accurately and promptly report malign influence following established protocols, ensuring all relevant details are documented and acted upon.

**Performance Steps**:

1. **Collaborate with Partner Organization to Document Instances of Malign Influence**
   - **References**: Documentation Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: Ensure secure documentation processes.
   - **Knowledge**: Understanding of documentation standards and requirements.
   - **Skills**: Documentation skills, attention to detail.
   - **Resources, Materials**: Access to documentation tools.
   - **Step Notes / Cues**: Record all relevant details accurately.

2. **Guide Partner Organization in Preparing and Submitting Reports**
   - **References**: Reporting Guidelines
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: Ensure secure transmission of reports.
   - **Knowledge**: Knowledge of reporting formats and protocols.
   - **Skills**: Reporting skills, attention to detail.
   - **Resources, Materials**: Access to reporting tools and platforms.
   - **Step Notes / Cues**: Ensure clarity and accuracy in reporting.

3. **Evaluate and Follow Up on Submitted Reports with Partner Organization**
   - **References**: Follow-Up Procedures
   - **Supporting and Supported Individual Tasks**: N/A
   - **Environmental and Safety**: N/A
   - **Knowledge**: Understanding of follow-up procedures and protocols.
   - **Skills**: Communication skills, attention to detail.
   - **Resources, Materials**: Access to follow-up tools and systems.
   - **Step Notes / Cues**: Verify that the report was received and acted upon.


---


### Skills and Knowledge Matrix

| Task                  | Knowledge                        | Skills                       |
|-----------------------|----------------------------------|------------------------------|
| Identify Misinformation and Disinformation | Understanding of credible sources, knowledge of source verification techniques, awareness of common bias indicators, familiarity with trusted sources | Analytical skills, attention to detail, critical thinking, research skills |
| Counter Misinformation and Disinformation  | Understanding of strategic communication, knowledge of misinformation tactics, techniques for countering disinformation, understanding of information dissemination channels, understanding of evaluation metrics, knowledge of reporting procedures | Strategic planning, analytical skills, communication skills, execution skills, reporting skills |
| Identify Rival Influence                  | Understanding of intelligence sources and methods, knowledge of influence techniques, understanding of geopolitical context, knowledge of reporting formats and requirements | Analytical skills, attention to detail, critical thinking, writing skills |
| Mitigate Rival Influence                  | Understanding of mitigation strategies and techniques, knowledge of influence mitigation techniques, understanding of evaluation metrics | Strategic planning, analytical skills, communication skills, execution skills, reporting skills |
| Create AI Generated Content               | Understanding of available AI tools and their applications, knowledge of data input techniques and settings configuration, understanding of quality standards for generated content | Technical skills, decision-making skills, attention to detail, review skills |
| Recognize AI Generated Content            | Understanding of AI-generated content characteristics, knowledge of available AI detection tools, understanding of content verification methods | Analytical skills, attention to detail, technical skills, tool proficiency, verification skills |
| Report Malign Influence                   | Understanding of documentation standards and requirements, knowledge of reporting formats and protocols, understanding of follow-up procedures and protocols | Documentation skills, attention to detail, reporting skills, communication skills |

### Performance Measures

#### Task 0001: Identify Misinformation and Disinformation

1. **Assess the Source of Information**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go

2. **Analyze the Content for Bias and Inconsistencies**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go

3. **Cross-Verify Information with Trusted Sources**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go

#### Task 0003: Identify Rival Influence

1. **Collect and Review Intelligence Reports**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go

2. **Analyze Patterns of Influence Activities**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go

3. **Prepare and Submit Reports**
   - **Sub-steps**: Yes
   - **Performance Measures**: Product
   - **Standard**: Go


#### Task 0005: Create AI Generated Content

1. **Select Appropriate AI Tools**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go

2. **Input Data and Configure Settings**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go

3. **Generate and Review Content**
   - **Sub-steps**: Yes
   - **Performance Measures**: Product
   - **Standard**: Go

#### Task 0006: Recognize AI Generated Content

1. **Analyze Media Content**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go

2. **Use AI Detection Tools**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go

3. **Cross-Verify with Known Authentic Content**
   - **Sub-steps**: Yes
   - **Performance Measures**: Process
   - **Standard**: Go


## Target Audience Analysis:

Create a Target Audience Profile (TAP) Analysis for the trainees who will engage in the training. This should include all critical characteristics needed to perform each critical task, including any prerequisite skills.

## Gap and Resource Analysis:

Perform a Gap and resource Analysis to update the scope statement and identify resources required to deliver training.  
Additional instructions are provided on the sheet.


### Approved Verbs
Adjust, Administer, Align, Analyze, Annotate, Apply, Approve, Assault, Assemble, Assess, Attack, Breach, Brief, Calculate, Camouflage, Challenge, Change, Check, Clear, Close, Collect, Communicate, Compare, Complete, Comply, Compute, Conduct, Configure, Confirm, Connect, Consolidate, Construct, Control, Coordinate, Correct, Correlate, Counsel, Counter, Cross, Debrief, Deconflict, Decontaminate, Defend, Define, Deliver, Demonstrate, Deploy, Designate, Destroy, Detect, Determine, Develop, Direct, Disassemble, Disconnect, Disengage, Dismantle, Dispatch, Displace, Distribute, Don, Download, Draft, Edit, Emplace, Employ, Enforce, Engage, Ensure, Enter, Erect, Escort, Establish, Estimate, Evacuate, Evade, Evaluate, Exchange, Extend, Extract, Facilitate, Fire, Forward, Fuel, Guard, Identify, Implement, Infiltrate, Inform, Initialize, Input, Inspect, Install, Integrate, Interpret, Inventory, Investigate, Issue, Land, Launch, Lay, Lead, Load, Localize, Locate, Lubricate, Maintain, Manage, Mark, Measure, Modify, Monitor, Mount, Move, Navigate, Negotiate, Neutralize, Notify, Observe, Obtain, Occupy, Open, Operate, Order, Organize, Orient, Pack, Patrol, Perform, Place, Plan, Plot, Position, Post, Predict, Prepare, Present, Prevent, Process, Produce, Project, Protect, Provide, Publish, React, Read, Receive, Recognize, Recommend, Reconnoiter, Record, Recover, Reduce, Refine, Register, Release, Relocate, Remove, Reorganize, Repair, Replace, Report, Request, Resolve, Restore, Retrieve, Review, Revise, Rig, Schedule, Secure, Select, Send, Set up, Store, Submit, Task, Test, Tow, Track, Train, Translate, Transmit, Transport, Treat, Troubleshoot, Turn, Unload, Update, Validate, Verify, Wear, Write, Zero



